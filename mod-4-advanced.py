...
def eta(first_stop, second_stop, route_map):
    start = [x[0] for x in route_map]
    destination = [y[1] for y in route_map]
    time = list(route_map.values())
    minutes = 0 
    minutes += time[start.index(first_stop)]["travel_time_mins"]
    ...
    if first_stop == 'upd' and second_stop == 'admu':
        ...
        while second_stop != destination[start.index(first_stop)]:
            first_stop = destination[start.index(first_stop)]
            minutes += time[start.index(first_stop)]["travel_time_mins"]
            print('It will take',minutes,'minutes to travel from', first_stop, 'to', second_stop)
            return minutes
print(dict(legs))
